...
class TweetExporter(object):
    ...
    def __init__(self, path, export_cols, aux_cols={}):
        ...
        # The path to export the tweet CSVs to.
    ...
    def export_tweet(self, tweet):
        # Get the functions to generate the column values.
        col_generators = [self.__getattribute__(c)
                          for c in self.export_cols]

        # Generate all of the column values.
        cols = [generator(tweet) for generator in col_generators]

        # Add the auxilliary values.
        cols = cols + self.aux_vals
